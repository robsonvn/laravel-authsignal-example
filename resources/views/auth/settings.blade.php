                    ...
                    <div class="grid gap-4">
                        <div class="flex items-center justify-between rounded-md border border-muted bg-background p-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-medium">Authentication App MFA</div>
                                    <div class="text-sm text-muted-foreground">Secure your account with a mobile app</div>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                @if (in_array('AUTHENTICATOR_APP', $enrolledVerificationMethods))
                                    <div class="rounded-full bg-green-500 px-2 py-1 text-xs font-medium text-green-50">Enrolled</div>
                                @else
                                    <div class="rounded-full bg-red-500 px-2 py-1 text-xs font-medium text-green-50">Not enrolled</div>
                                @endif
                                <form method="POST" action="{{route('auth.enroll-authenticator-app')}}">
                                    @csrf
                                    <button
                                        class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium h-9 rounded-md px-3 border border-input">
                                        Enroll
                                    </button>
                                </form>
                            </div>
